/* ========================================================================
   ...
   Module 08: CDK Service Suite
   ======================================================================== */

const MODULE_KEY = "dyer_cdk_suite_progress_v1";
...
function markComplete(lessonId) {
  ...
  if (window.dyerTraining && window.dyerTraining.recordLessonComplete) {
    window.dyerTraining.recordLessonComplete("cdk-suite", lessonId)
      .catch(e => console.warn("[cdk-suite] recordLessonComplete failed:", e));
  }
}
async function hydrateProgressFromDb() {
  ...
  try {
    const [res, quizRes] = await Promise.all([
      window.dyerTraining.fetchMyProgress("cdk-suite"),
      window.dyerTraining.fetchMyQuizAttempts
        ? window.dyerTraining.fetchMyQuizAttempts("cdk-suite")
...
function resetProgress() {
  if (confirm("Reset all module progress, quiz scores, and checklists? This cannot be undone.")) {
    ...
    Object.keys(localStorage).filter(k => k.startsWith("dyer_cdk_suite_")).forEach(k => localStorage.removeItem(k));
    location.reload();
  }
}
...
function initChecklists() {
  document.querySelectorAll(".checklist").forEach(list => {
    const key = "dyer_cdk_suite_chk_" + (list.getAttribute("data-checklist") || "default");
    let state = {};
  ...
  if (submit) submit.addEventListener("click", () => {
    ...
    if (passEl) {
      passEl.textContent = pct >= 80
        ? `You scored ${right} of ${totalQ}. You're ready to work the CDK Service Suite workflow with confidence.`
    ...
    if (window.dyerTraining && window.dyerTraining.recordQuizAttempt) {
      window.dyerTraining.recordQuizAttempt({
        moduleId: "cdk-suite", score: pct, totalQuestions: totalQ, correctAnswers: right
      }).catch(e => console.warn("[cdk-suite] recordQuizAttempt failed:", e));
    }
...
function initScorecard() {
  ...
  if (!card) return;
  const KEY = "dyer_cdk_suite_scorecard";
  let saved = {};
