/* ========================================================================
   ...
   Module 03: Overcoming Customer Objections
   Shared scripts: progress, checklists, quiz, roleplay, scorecard
   ======================================================================== */

const MODULE_KEY = "dyer_objections_progress_v1";
...
function markComplete(lessonId) {
  ...
  if (window.dyerTraining && window.dyerTraining.recordLessonComplete) {
    window.dyerTraining
      .recordLessonComplete("objections", lessonId)
      .catch(e => console.warn("[objections] recordLessonComplete failed:", e));
  }
}
...
function resetProgress() {
  if (confirm("Reset all module progress, quiz scores, and checklists? This cannot be undone.")) {
    ...
    Object.keys(localStorage)
      .filter(k => k.startsWith("dyer_objections_"))
...
function initChecklists() {
  document.querySelectorAll(".checklist").forEach(list => {
    const key = "dyer_objections_chk_" + (list.getAttribute("data-checklist") || "default");
    let state = {};
  ...
  if (submit) submit.addEventListener("click", () => {
    ...
    if (passEl) {
      passEl.textContent = pct >= 80
        ? `You scored ${right} of ${totalQ}. You're ready to handle these objections at the drive.`
    ...
    if (window.dyerTraining && window.dyerTraining.recordQuizAttempt) {
      window.dyerTraining
        ...
        .catch(e => console.warn("[objections] recordQuizAttempt failed:", e));
    }
...
function initScorecard() {
  ...
  if (!card) return;
  const KEY = "dyer_objections_scorecard";
  let saved = {};
