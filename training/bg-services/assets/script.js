/* ========================================================================
   ...
   Module 07: BG Maintenance Services
   ======================================================================== */

const MODULE_KEY = "dyer_bg_services_progress_v1";
...
function markComplete(lessonId) {
  ...
  if (window.dyerTraining && window.dyerTraining.recordLessonComplete) {
    window.dyerTraining.recordLessonComplete("bg-services", lessonId)
      .catch(e => console.warn("[bg-services] recordLessonComplete failed:", e));
  }
}
async function hydrateProgressFromDb() {
  ...
  try {
    const [res, quizRes] = await Promise.all([
      window.dyerTraining.fetchMyProgress("bg-services"),
      window.dyerTraining.fetchMyQuizAttempts
        ? window.dyerTraining.fetchMyQuizAttempts("bg-services")
    ...
    if (res && res.ok && Array.isArray(res.data)) {
      ...
      if (merged.length !== (p.completed || []).length) { p.completed = merged; changed = true; }
    }
...
function resetProgress() {
  if (confirm("Reset all module progress, quiz scores, and checklists? This cannot be undone.")) {
    ...
    Object.keys(localStorage).filter(k => k.startsWith("dyer_bg_services_")).forEach(k => localStorage.removeItem(k));
    location.reload();
  }
}
...
function initChecklists() {
  document.querySelectorAll(".checklist").forEach(list => {
    const key = "dyer_bg_services_chk_" + (list.getAttribute("data-checklist") || "default");
    let state = {};
  ...
  if (submit) submit.addEventListener("click", () => {
    ...
    if (passEl) {
      passEl.textContent = pct >= 80
        ? `You scored ${right} of ${totalQ}. You're ready to recommend BG maintenance services with confidence.`
    ...
    if (window.dyerTraining && window.dyerTraining.recordQuizAttempt) {
      window.dyerTraining.recordQuizAttempt({
        moduleId: "bg-services", score: pct, totalQuestions: totalQ, correctAnswers: right
      }).catch(e => console.warn("[bg-services] recordQuizAttempt failed:", e));
    }
...
function initScorecard() {
  ...
  if (!card) return;
  const KEY = "dyer_bg_services_scorecard";
  let saved = {};
