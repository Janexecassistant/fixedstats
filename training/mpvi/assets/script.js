/* ========================================================================
   ...
   Module 02: Technician Video MPVI
   Shared scripts: progress, checklists, quiz, video critique, scorecard
   ======================================================================== */

const MODULE_KEY = "dyer_mpvi_progress_v1";
...
async function hydrateProgressFromDb() {
  ...
  try {
    const [res, quizRes] = await Promise.all([
      window.dyerTraining.fetchMyProgress("mpvi"),
      window.dyerTraining.fetchMyQuizAttempts
        ? window.dyerTraining.fetchMyQuizAttempts("mpvi")
...
function resetProgress() {
  if (confirm("Reset all module progress, quiz scores, and checklists? This cannot be undone.")) {
    ...
    Object.keys(localStorage)
      .filter(k => k.startsWith("dyer_mpvi_"))
...
function initChecklists() {
  document.querySelectorAll(".checklist").forEach(list => {
    const key = "dyer_mpvi_chk_" + (list.getAttribute("data-checklist") || "default");
    let state = {};
  ...
  if (submit) submit.addEventListener("click", () => {
    ...
    if (passEl) {
      passEl.textContent = pct >= 80
        ? `You scored ${right} of ${totalQ}. Module mastery confirmed. Time to record some videos.`
...
/* ---------------- Video Critique (same UX as roleplay) ---------------- */
