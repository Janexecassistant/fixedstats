/* ========================================================================
   ...
   Module 04: Menu Sales
   ======================================================================== */

const MODULE_KEY = "dyer_menu_sales_progress_v1";
...
function markComplete(lessonId) {
  ...
  if (window.dyerTraining && window.dyerTraining.recordLessonComplete) {
    window.dyerTraining.recordLessonComplete("menu-sales", lessonId)
      .catch(e => console.warn("[menu-sales] recordLessonComplete failed:", e));
  }
}
async function hydrateProgressFromDb() {
  ...
  try {
    const [res, quizRes] = await Promise.all([
      window.dyerTraining.fetchMyProgress("menu-sales"),
      window.dyerTraining.fetchMyQuizAttempts
        ? window.dyerTraining.fetchMyQuizAttempts("menu-sales")
...
function resetProgress() {
  if (confirm("Reset all module progress, quiz scores, and checklists? This cannot be undone.")) {
    ...
    Object.keys(localStorage).filter(k => k.startsWith("dyer_menu_sales_")).forEach(k => localStorage.removeItem(k));
    location.reload();
  }
}
...
function initChecklists() {
  document.querySelectorAll(".checklist").forEach(list => {
    const key = "dyer_menu_sales_chk_" + (list.getAttribute("data-checklist") || "default");
    let state = {};
  ...
  if (submit) submit.addEventListener("click", () => {
    ...
    if (passEl) {
      passEl.textContent = pct >= 80
        ? `You scored ${right} of ${totalQ}. You're ready to use the menu in CDK with confidence.`
    ...
    if (window.dyerTraining && window.dyerTraining.recordQuizAttempt) {
      window.dyerTraining.recordQuizAttempt({
        moduleId: "menu-sales", score: pct, totalQuestions: totalQ, correctAnswers: right
      }).catch(e => console.warn("[menu-sales] recordQuizAttempt failed:", e));
    }
...
function initScorecard() {
  ...
  if (!card) return;
  const KEY = "dyer_menu_sales_scorecard";
  let saved = {};
