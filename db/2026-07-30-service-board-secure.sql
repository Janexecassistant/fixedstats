...
begin
 ...
 r.id:=coalesce(r.id,gen_random_uuid()); r.created_by:=coalesce(r.created_by,uid); r.updated_by:=uid; r.created_at:=coalesce(r.created_at,now()); r.status_changed_at:=coalesce(r.status_changed_at,r.updated_at,r.created_at,now()); r.updated_at:=now();
