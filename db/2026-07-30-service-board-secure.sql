...
create table if not exists public.service_board_ros (
  ...
  customer_name text not null default '', vehicle text not null default '', needs_rental boolean not null default false, rental_type text not null default '',
  ...
  estimated_hours numeric(5,2) not null default 1, closed_hours numeric(7,2) not null default 0, job_type text not null default 'repair', required_skill text not null default '',
  ...
  hold_reason text not null default '', status text not null default 'checked_in', status_changed_at timestamptz not null default now(), notes text not null default '',
...
alter table public.service_board_ros add column if not exists needs_rental boolean not null default false;
...
begin
  ...
  if v_user is null or (
    lower(coalesce(v_user.role,'')) not in ('admin','owner','fixed_ops','executive')
    and v_user.default_store is distinct from p_store_id
    and not exists(select 1 from user_store_access where user_id=v_user.id and store_id=p_store_id)
...
begin
 ...
 r.id:=coalesce(r.id,gen_random_uuid()); r.created_by:=coalesce(r.created_by,uid); r.updated_by:=uid; r.created_at:=coalesce(r.created_at,now()); r.status_changed_at:=coalesce(r.status_changed_at,r.updated_at,r.created_at,now()); r.closed_hours:=case when r.status='completed' and coalesce(r.closed_hours,0)=0 then greatest(coalesce(r.estimated_hours,0),0) else coalesce(r.closed_hours,0) end; r.updated_at:=now();
 insert into service_board_ros as t select (r).* on conflict(store_id,ro_number) do update set customer_name=excluded.customer_name,vehicle=excluded.vehicle,needs_rental=excluded.needs_rental,rental_type=excluded.rental_type,rental_due_date=excluded.rental_due_date,check_in_date=excluded.check_in_date,parts_status=excluded.parts_status,advisor_name=excluded.advisor_name,pay_type=excluded.pay_type,technician=excluded.technician,technician_user_id=excluded.technician_user_id,estimated_hours=excluded.estimated_hours,closed_hours=excluded.closed_hours,job_type=excluded.job_type,required_skill=excluded.required_skill,priority=excluded.priority,promised_at=excluded.promised_at,dispatch_status=excluded.dispatch_status,dispatched_at=excluded.dispatched_at,dispatched_by=excluded.dispatched_by,hold_reason=excluded.hold_reason,status_changed_at=case when t.status is distinct from excluded.status then now() else t.status_changed_at end,status=excluded.status,notes=excluded.notes,source=excluded.source,updated_by=uid,updated_at=now(),completed_at=excluded.completed_at returning t.* into saved; return saved;
end $$;
...
begin
 select * into r from jsonb_populate_record(null::public.rental_flow_cars,p_row); uid:=board_authorize(p_token,r.store_id,true); r.id:=coalesce(r.id,gen_random_uuid()); r.created_by:=coalesce(r.created_by,uid); r.updated_by:=uid; r.created_at:=coalesce(r.created_at,now()); r.updated_at:=now();
